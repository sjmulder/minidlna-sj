...
struct magic_container_s magic_containers[] =
{
	...
	{ "Recently Played",
	  ...
	  "\"1$FF1\"",
	  "o.OBJECT_ID",
	  "(SELECT h.client_mac FROM history h LEFT JOIN details d ON (h.detail_id = d.id) WHERE mime glob 'a*' ORDER BY atime DESC) WHERE req_mac = client_mac LIMIT 50",
		"LEFT JOIN history h ON (h.detail_id = d.id)",
	  "mime glob 'a*' AND ref_id is NULL AND h.client_mac = req_mac",
	...
	{ "Recently Played",
	  ...
	  "\"2$FF1\"",
	  "o.OBJECT_ID",
	  "(SELECT h.client_mac FROM history h LEFT JOIN details d ON (h.detail_id = d.id) WHERE mime glob 'v*' ORDER BY atime DESC) WHERE req_mac = client_mac LIMIT 50",
		"LEFT JOIN history h ON (h.detail_id = d.id)",
	  "mime glob 'v*' AND ref_id is NULL AND h.client_mac = req_mac",
		"ORDER BY h.atime DESC",
	  50,
	  0,
	},
	...
	{ "Recently Viewed",
	  ...
	  "\"3$FF1\"",
	  "o.OBJECT_ID",
	  "(SELECT h.client_mac FROM history h LEFT JOIN details d ON (h.detail_id = d.id) WHERE mime glob 'i*' ORDER BY atime DESC) WHERE req_mac = client_mac LIMIT 50",
		"LEFT JOIN history h ON (h.detail_id = d.id)",
	  "mime glob 'i*' AND ref_id is NULL AND h.client_mac = req_mac",
		"ORDER BY h.atime DESC",
	  50,
	  0,
	},
